...
export default async function handler(req, res) {
  ...
  const { data: positions } = await supabaseAdmin
    .from('positions').select('*').eq('inst_id', inst_id).order('reg_number')
  ...
  const rows = (positions || []).map(p => {
    ...
    const statusObj = STATUSES.find(s => s.id === st?.status_id)
    let statusLabel = statusObj?.label || ''
    if (st?.status_id === 'nk_prep' && st?.nk_percent != null) {
      statusLabel = `Подготовка к НК на ${st.nk_percent}%`
    }
    return {
      'Установка':        inst.name,
  ...
  res.setHeader('Content-Disposition', `attachment; filename="${encodeURIComponent(inst.name)}_statuses.xlsx"`)
